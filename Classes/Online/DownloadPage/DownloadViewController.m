...
-(void) makeUrl
{
    NSMutableString *tempUrl =[NSMutableString stringWithString:@"http://www.cbeta.org/download/pdf/"];
...
- (void)_receiveDidStopWithStatus:(NSString *)statusString
{
    ...
    [[CbetaIpadAppDelegate  sharedAppDelegate] didStopNetworking];
	...
	NSRange range = [m_volumnName rangeOfString:@" "];
	NSString *m_volumnNo = [m_volumnName substringToIndex:range.location];
	NSString* unzipto = [[tempZipName stringByAppendingString:m_volumnNo] stringByAppendingString:@"/"] ;
...
// Starts a connection to download the current URL.
{
    ...
    url = [[CbetaIpadAppDelegate sharedAppDelegate] smartURLForString:kDefaultGetURLText];
   ...
   //   self.filePath = [[cbetaAppDelegate sharedAppDelegate] pathForTemporaryFileWithPrefix:@"Get"];
		  self.filePath = [[CbetaIpadAppDelegate sharedAppDelegate] pathForTemporaryFileWithPrefix:self.m_volumnName];
        assert(self.filePath != nil);
...
#pragma unused(theConnection)
    ...
    do {
        ...
        if (bytesWritten == -1) {
			...
			NSMutableString *downingStatus =[NSMutableString stringWithFormat:@"正在下載%@",self.m_volumnName];
...
#pragma unused(error)
   ...
   // [self _stopReceiveWithStatus:@"Connection failed"];
	[self _stopReceiveWithStatus:@"下載失敗，請檢查網絡連接！"];
}
...
#pragma unused(sender)
    if (self.isReceiving) {
       // [self _stopReceiveWithStatus:@"Cancelled"];
		[self _stopReceiveWithStatus:@"取消下載"];
    } else {
        [self _startReceive];
    }
}
...
{    
    [super viewDidLoad];
	self.contentSizeForViewInPopover = CGSizeMake(320, 367);
