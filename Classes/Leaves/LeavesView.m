...
- (void) setUpLayers {
    ...
    topPage.contentsScale = [[UIScreen mainScreen] scale]*2;
    ...
    topPageOverlay.contentsScale = [[UIScreen mainScreen] scale]*2; 
    ...
    topPageShadow.contentsScale = [[UIScreen mainScreen] scale]*2; 
    ...
    topPageReverse.contentsScale = [[UIScreen mainScreen] scale]*2; 
    ...
    topPageReverseImage.contentsScale = [[UIScreen mainScreen] scale]*2; 
	topPageReverseImage.masksToBounds = YES;
	
	topPageReverseOverlay = [[CALayer alloc] init];
    topPageReverseOverlay.contentsScale = [[UIScreen mainScreen] scale]*2;
	
	topPageReverseShading = [[CAGradientLayer alloc] init];
    topPageReverseShading.contentsScale = [[UIScreen mainScreen] scale]*2;
    ...
    bottomPage.contentsScale = [[UIScreen mainScreen] scale]*2;
    ...
    bottomPageShadow.contentsScale = [[UIScreen mainScreen] scale]*2;
    ...
    leftPage.contentsScale = [[UIScreen mainScreen] scale]*2;
    ...
    leftPageOverlay.contentsScale = [[UIScreen mainScreen] scale]*2;
...
- (void) willTurnToPageAtIndex:(NSUInteger)index {
	if ([mydelegate respondsToSelector:@selector(leavesView:willTurnToPageAtIndex:)])
		[mydelegate leavesView:self willTurnToPageAtIndex:index];
}

- (void) didTurnToPageAtIndex:(NSUInteger)index {
	if ([mydelegate respondsToSelector:@selector(leavesView:didTurnToPageAtIndex:)])
		[mydelegate leavesView:self didTurnToPageAtIndex:index];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if ([self touchedPrevPage] && [self hasPrevPage]) {		
		...
		[CATransaction commit];
		touchIsActive = YES;
		if ([mydelegate respondsToSelector:@selector(touchIsActiveInLeavesView:)]) {
			[mydelegate touchIsActiveInLeavesView:self];
		}
	...
	{
		touchIsActive = NO;
		if ([mydelegate respondsToSelector:@selector(touchNotActiveInLeavesView:)]) {
			[mydelegate touchNotActiveInLeavesView:self];
		}
	}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	BOOL allTouchEnd = ([touches count] == [[event touchesForView:self] count]);
	
	if (!touchIsActive) {
		if (!isMultiTouch) {
			if ([mydelegate respondsToSelector:@selector(gotsingleTap)]) {
				[mydelegate gotsingleTap];
			}
		}
		return;
	}
